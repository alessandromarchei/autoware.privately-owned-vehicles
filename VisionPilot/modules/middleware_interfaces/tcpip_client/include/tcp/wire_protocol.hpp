...
constexpr std::size_t WIRE_RESULT_FIXED_SIZE = 676;
...
inline std::vector<std::uint8_t> encode_result(const common::VisionPilotOutput& result)
{
    ...
    writer.writeF64(result.inference.visionpilot_ms);       //24-31
...
inline bool decode_result(const std::vector<std::uint8_t>& wire, common::VisionPilotOutput& result)
{
    ...
    if (!reader.readU64(decoded.inference.frame_id) ||
        !reader.readF64(decoded.inference.wall_ms) ||
        !reader.readF64(decoded.inference.pre_ms) ||
        !reader.readF64(decoded.inference.visionpilot_ms)) {
        return false;
    }
