...
// Execute function for multi-job-container usecases
int32_t execute(R_EXFWK * exfwk, std::vector<JobContainer *> & job_containers)
{
    uint8_t job_container_num = (uint8_t)job_containers.size();
    container_counter         = job_container_num;
    uint32_t timeout_u32      = 0;
    for (uint8_t job_container_idx = 0; job_container_idx < job_container_num; job_container_idx++)
    {
        ...
        uint32_t         parser_container_id;
        e_exfwk_return_t exfwk_ret =
            exfwk->push(*job_containers[job_container_idx], 1, callback_info, parser_container_id);
        if (exfwk_ret != RETURN_EXFWK_OK)
        {
            printf("Failed to push jobs for container %d\n", job_container_idx);
            return -1;
        }
    }
...
void time_evaluate(R_EXFWK *exfwk, JobContainer *&job_container, std::vector<Network *> &networks,
                   int repeat_number, std::vector<LatencyStat> &execute_time,
                   std::vector<std::vector<uint8_t>> core_list, bool warm_up = false) {
...
// Time evaluation function for single-NN multi-job-container usecases
void time_evaluate(R_EXFWK *exfwk, std::vector<JobContainer *> &job_containers, std::vector<PipelineId> pid,
                   Network *network, int repeat_number, std::vector<LatencyStat> &execute_time,
                   std::vector<std::vector<uint8_t>> core_list, bool warm_up = false) {
  st_osal_time_t start;
  st_osal_time_t end;
  osal_nano_sec_t timestamp;

  // Duplicate core for single NN singel core multi containers (pipelie application)
  // i.e., {{0}} -> {{0,0}} if 2 pipelines
  if (!core_list[0].empty() && core_list[0].size() < pid.size()) {
    size_t original_size = core_list[0].size();
    for (size_t i = 0; i < pid.size() - original_size; ++i) {
      core_list[0].push_back(core_list[0][0]);
    }
  }

  if (warm_up) {
    for (PipelineId p : pid) {
      network->syncIO(e_sync_direction_t::H2D, p);
    }

    auto exec_ret = execute(exfwk, job_containers);
    if (exec_ret != e_hycoah_return_t::RETURN_HYCOAH_OK) {
      std::cout << " EXECUTION FAILED " << std::endl;
      return;
    }
    ...
    for (PipelineId p : pid) {
      network->syncIO(e_sync_direction_t::D2H, p);
    }
  }
  ...
  while (number++ < repeat_number) {
    ...
    uint8_t pid_index = 0;
    for (PipelineId p : pid) {
      auto latency = network->getLatency(p);
      cnnip_time += latency[0];
      if (!core_list[0].empty()) {
        dsp_time[core_list[0][pid_index]] += latency[1];
      } else {
        if (latency[1] > 0) {
          throw std::runtime_error("DSP core(s) are not correctly specified in msgpack artifact folder name.");
        }
      }
      pid_index = static_cast<uint8_t>(pid_index + 1);
    }
